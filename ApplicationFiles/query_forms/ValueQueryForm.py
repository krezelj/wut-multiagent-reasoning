...
from core.queries import ExecutabilityQuery, ValueQuery
from core.statements import Effect


class ValueQueryForm(QueryForm):
    def __init__(self, root):
        super().__init__(root)
        self.statement_form_frame = LabelFrame(root, text="Value query", width=500, font=15)  # Set the width
        ...
        self.condition1_label = Label(self.statement_form_frame, text="after")
        ...
        self.condition1_entry.grid(row=0, column=1)
        self.condition2_label.grid(row=2, column=0)
        self.condition2_entry.grid(row=2, column=1)
    ...
    def answer_query(self, agents) -> Any:
        ...
        return ValueQuery(necessary, condition1, Program.parse_string(program, agents), condition2)
